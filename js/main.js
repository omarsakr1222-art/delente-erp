...
// --- Cloud Fixes: ensureRawAndPackFromState + saveCostListsToFirebase ---
// Lightweight redefinitions to preserve app behavior after safe split.

function serverTs(){
    try { return firebase.firestore.FieldValue.serverTimestamp(); } catch(_) { return new Date().toISOString(); }
}

async function waitForDbReady(timeoutMs){
    const start = Date.now();
    while (!window.db) {
        if (Date.now() - start > (timeoutMs || 3000)) break;
...
window.ensureRawAndPackFromState = function(){
    try {
        ...
        window.costRaw = Array.isArray(window.costRaw) ? window.costRaw : [];
        window.costPack = Array.isArray(window.costPack) ? window.costPack : [];
        const seenPack = new Set(window.costPack.map(i => (i?.name||'').toLowerCase()));
        products.forEach(p => {
            const name = (p?.name||'').trim(); if (!name) return;
            const cat = (p?.category||'').toString().toLowerCase();
            if (/تعبئة|تغليف|عبوة|ورق|كيس/.test(cat)) {
                const key = name.toLowerCase();
                if (!seenPack.has(key)) { window.costPack.push({ name, cost: 0 }); seenPack.add(key); }
            }
        });
        ['ورق تعبئة','اكياس','اكواب','علب كرتون'].forEach(name => {
            const key = name.toLowerCase(); if (!seenPack.has(key)) { window.costPack.push({ name, cost: 0 }); seenPack.add(key); }
        });
        console.log('ensureRawAndPackFromState:', { raw: (window.costRaw||[]).length, pack: (window.costPack||[]).length });
    } catch(e){ console.warn('ensureRawAndPackFromState failed', e); }
};

window.saveCostListsToFirebase = async function(immediate){
    try {
        if (!window.db) { await waitForDbReady(5000); }
        if (!window.db) { console.warn('saveCostListsToFirebase: Firestore not ready'); return false; }
        ...
        const ops = Array.isArray(window.costOps) ? window.costOps : [];
        const payload = { rawMaterials: raw, packaging: pack, finished: fin, operations: ops, updatedAt: serverTs() };
        if (!immediate && raw.length === 0 && pack.length === 0 && fin.length === 0 && ops.length === 0) {
            try { const snap = await ref.get(); if (snap && snap.exists) { console.warn('saveCostListsToFirebase: skip empty save'); return true; } } catch(_){}
        }
        await ref.set(payload, { merge: true });
        console.log('✅ Cost lists saved', { raw: raw.length, pack: pack.length, fin: fin.length, ops: ops.length });
        return true;
    } catch(e){ console.warn('saveCostListsToFirebase failed', e); return false; }
};
